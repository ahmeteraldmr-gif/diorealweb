<?php
    ...
    $seo_title = ($locale === 'en')
        ? ($restoran->seo_title_en ?: ($restoran->name['en'] ?? 'Detay') . ' - Dioreal')
        : ($restoran->seo_title_tr ?: ($restoran->name['tr'] ?? 'Detay') . ' - Dioreal');
    $seo_desc = ($locale === 'en')
        ? ($restoran->seo_description_en ?: \Illuminate\Support\Str::limit(strip_tags($restoran->desc['en'] ?? ''), 155))
        : ($restoran->seo_description_tr ?: \Illuminate\Support\Str::limit(strip_tags($restoran->desc['tr'] ?? ''), 155));
    $og_image = $restoran->og_image ? asset($restoran->og_image) : asset($restoran->img);
    $canonical = $canonical ?? route('restoran.detay', $restoran->slug_tr ?: $restoran->id);
    $noindex = $restoran->seo_noindex;
    
    $hreflang_tr = $hreflang_tr ?? route('restoran.detay', $restoran->slug_tr ?: $restoran->id);
    $hreflang_en = $hreflang_en ?? ($restoran->slug_en ? route('restoran.detay', $restoran->slug_en) : null);
    $og_type = 'Restaurant' == 'Article' ? 'article' : 'website';

    $schema_json = '<script type="application/ld+json">
    {
      "@context": "https://schema.org",
      "@type": "Restaurant",
      "name": "'.addslashes($restoran->name['tr'] ?? '').'",
...
<body>
    ...
    <nav id="mainNav">
        ...
        <ul class="nav-links">
            ...
            <li><a href="<?php echo e(url('/restoranlar')); ?>" class="active-page" data-i18n="nav_restaurants">Restoranlar</a></li>
    ...
    ?>
    <div class="page-hero" style="<?php if(!$showVideoCover): ?> background-image: url('<?php echo e($restImgUrl); ?>'); <?php endif; ?>">
        <?php if($showVideoCover): ?>
            <div class="hero-video-container" style="position: absolute; inset: 0; width: 100%; height: 100%; overflow: hidden; z-index: 0;">
                <?php if(!empty($restoran->video_file)): ?>
                    <video src="<?php echo e(asset($restoran->video_file)); ?>" autoplay loop muted playsinline style="width: 100%; height: 100%; object-fit: cover;"></video>
        ...
        <div class="page-hero-content" style="position: relative; z-index: 2;">
            <span class="page-eyebrow lang-text-tr"><?php echo e($restoran->tag['tr'] ?? ''); ?></span>
            <span class="page-eyebrow lang-text-en"><?php echo e($restoran->tag['en'] ?? ''); ?></span>
            <h1 class="page-title lang-text-tr"><?php echo e($restoran->name['tr'] ?? ''); ?></h1>
            <h1 class="page-title lang-text-en"><?php echo e($restoran->name['en'] ?? ''); ?></h1>
        </div>
    </div>
        ...
        <div class="detail-grid">
            ...
            <div class="detail-story reveal">
                ...
                <div class="lang-text-tr">
                    <?php echo nl2br($restoran->long_desc['tr'] ?? ($restoran->desc['tr'] ?? '')); ?>

                </div>
                <div class="lang-text-en">
                    <?php echo nl2br($restoran->long_desc['en'] ?? ($restoran->desc['en'] ?? '')); ?>
            ...
            <div class="detail-sidebar-card reveal" style="transition-delay: 0.2s">
                ...
                <div class="sidebar-info-item">
                    ...
                    <div>
                        ...
                        <div class="sidebar-info-value lang-text-tr"><?php echo e($restoran->location['tr'] ?? ($restoran->destination->name['tr'] ?? ($restoran->tag['tr'] ?? ''))); ?></div>
                        <div class="sidebar-info-value lang-text-en"><?php echo e($restoran->location['en'] ?? ($restoran->destination->name['en'] ?? ($restoran->tag['en'] ?? ''))); ?></div>
                    </div>
                </div>
                ...
                <div class="sidebar-info-item">
                    ...
                    <div>
                        ...
                        <div class="sidebar-info-value"><?php echo e($settings['contact_email'] ?? 'info@dioreal.com'); ?></div>
                    </div>
                </div>
                ...
                <div class="sidebar-info-item">
                    ...
                    <div>
                        ...
                        <div class="sidebar-info-value"><?php echo e($settings['contact_phone'] ?? '+90 532 000 0000'); ?></div>
                    </div>
                </div>

                <a href="https://wa.me/<?php echo e(format_whatsapp($settings['whatsapp'] ?? '')); ?>?text=Merhaba,%20<?php echo e(urlencode($restoran->name['tr'] ?? $restoran->name['en'] ?? 'Restoran')); ?>%20hakkında%20detaylı%20bilgi%20ve%20masa%20rezervasyon%20talebinde%20bulunmak%20istiyorum." 
                   target="_blank" 
                   class="btn-booking">
    ...
    <?php if(!empty($restoran->video_file) || !empty($restoran->video_url)): ?>
        <section class="video-section reveal" style="max-width: 1200px; margin: 4rem auto; padding: 0 2rem;">
            ...
            <div class="video-container" style="position: relative; border-radius: 16px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.15); background: #000; aspect-ratio: 16/9; max-width: 900px; margin: 0 auto;">
                <?php if(!empty($restoran->video_file)): ?>
                    <video src="<?php echo e(asset($restoran->video_file)); ?>" controls style="width: 100%; height: 100%; object-fit: cover;"></video>
                <?php elseif(!empty($restoran->video_url)): ?>
                    <?php
                        $embedUrl = $restoran->video_url;
                        if (preg_match('/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/ ]{11})/', $restoran->video_url, $matches)) {
    ...
    <section class="gallery-section reveal">
        ...
        <div class="gallery-grid">
            <?php if(!empty($restoran->gallery) && is_array($restoran->gallery)): ?>
                <?php $__currentLoopData = $restoran->gallery; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $g): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="gallery-img-wrapper">
                        <img src="<?php echo e(str_starts_with($g, 'data:') || str_starts_with($g, 'http') ? $g : asset($g)); ?>" alt="Mekan Görseli">
                    </div>
...
<?php /**PATH C:\Users\ahmet\Desktop\dioreal web\resources\views\restoran-detay.blade.php ENDPATH**/ ?>